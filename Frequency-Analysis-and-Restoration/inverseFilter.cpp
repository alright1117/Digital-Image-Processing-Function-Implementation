...
Mat ideaLowPassFilter(Mat complexImg)
{
	...
	for (int i = 0; i < m; i++)
	{
		for (int j = 0; j < n; j++)
		{
			...
			if (dist > s)
			{
				resultImg.at<Vec2f>(i, j)[0] /= sqrt(dist);
				resultImg.at<Vec2f>(i, j)[1] /= sqrt(dist);
			}
			else
			{
...
int main(int argc, const char** argv)
{
	...
	// save image
	imwrite("result/denoise/inverse/mag.png", mag8UC);
	imwrite("result/denoise/inverse/filterMag.png", filterMag8UC);
	imwrite("result/denoise/inverse/filterImg.png", finalImg8UC);
