...
void Compressor::forwardDCT() {
	...
	dctImage = cv::Mat_<uchar>(height, width);
	...
	for (int i = 0; i < height; i += 8) {
		for (int j = 0; j < width; j += 8) {
			...
			//std::cout << "Copyto\n";
			blockC.copyTo(dctImage(cv::Rect(j, i, 8, 8)));
...
void Compressor::inverseDCT() {
	...
	for (int i = 0; i < height; i += 8) {
		for (int j = 0; j < width ; j += 8) {
			// Get a block 8x8 for each position on image
			cv::Mat block = dctImage(cv::Rect(j, i, 8, 8));
			...
			//std::cout << "Copyto\n";
			block8.copyTo(grayImage(cv::Rect(j, i, 8, 8)));
		}
	}
...
void Compressor::decompress(char* fileName) {
	...
	dctImage = cv::Mat::zeros(height, width, CV_8UC1);
